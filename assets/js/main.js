...
// 2. Randomize gallery items
const $gallery = $('.td-gallery-grid');
const $items = $gallery.children('.col-lg-4').get();

// Shuffle function
$items.sort(() => Math.random() - 0.5);

// Append shuffled items back into the gallery container
...
// Ahsan Gallery Filter with Pagination and Animation
document.addEventListener("DOMContentLoaded", function () {
  const buttons = document.querySelectorAll(".ahsan-filter-btn");
  const grid = document.querySelector(".ahsan-gallery-grid");
  ...
  function displayItems(page = 1) {
    ...
    pageItems.forEach((item) => {
      item.classList.add("ahsan-fade-in");
  ...
  function renderPagination() {
    const existingPagination = document.querySelector(".ahsan-pagination");
    ...
    pagination.classList.add("ahsan-pagination", "text-center", "mt-4");

    const prevBtn = document.createElement("button");
    prevBtn.textContent = "Prev";
    prevBtn.classList.add("ahsan-page-btn");
    if (currentPage === 1) prevBtn.disabled = true;

    const nextBtn = document.createElement("button");
    nextBtn.textContent = "Next";
    nextBtn.classList.add("ahsan-page-btn");
  ...
  function changePage(page) {
    grid.classList.add("ahsan-page-fade");
    setTimeout(() => {
      grid.classList.remove("ahsan-page-fade");
      currentPage = page;
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (filter === "all") {
        filteredItems = shuffle([...items]); // shuffle all together
      } else {
        filteredItems = shuffle(
          items.filter((item) => item.getAttribute("data-category") === filter)
        );
      }
